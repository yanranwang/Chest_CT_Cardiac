#!/bin/bash
#SBATCH --job-name=cardiac_training_a6000
...
show_help() {
    ...
    echo ""
    echo "选项:"
    echo "  --config FILE         配置文件路径 (默认: configs/multi_gpu_training_config.json)"
    ...
    echo ""
    echo "示例:"
    echo "  sbatch $0 --config configs/multi_gpu_training_config.json --epochs 100"
    echo "  sbatch $0 --output_dir outputs/a6000_experiment --batch_size 32"
}
...
# 设置默认值
CONFIG_FILE=${CONFIG_FILE:-"configs/multi_gpu_training_config.json"}

# 如果没有指定输出目录，自动生成唯一目录
if [ -z "$OUTPUT_DIR" ]; then
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    OUTPUT_DIR="outputs/a6000_cardiac_training_${TIMESTAMP}_job${SLURM_JOB_ID}"
fi
...
# 打印作业信息
echo "========================================"
echo "心脏功能训练作业信息 (A6000 GPU)"
echo "========================================"
echo "作业ID: $SLURM_JOB_ID"
echo "作业名称: $SLURM_JOB_NAME"
echo "节点: $SLURM_JOB_NODELIST (amalfi - A6000)"
echo "分区: $SLURM_JOB_PARTITION"
echo "GPU: 2x A6000 (48GB VRAM each)"
